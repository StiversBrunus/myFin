
```text
Bancos 🏛

├── Bancos Tradicionais 
│   ├── Banco do Brasil ✅
│   ├── Caixa Econômica Federal ✅
│   ├── Bradesco ✅
│   ├── Itaú Unibanco ✅
│   └── Santander ✅
│
├── Bancos Digitais
│   ├── Nubank ✅
│   ├── Banco Inter ✅
│   ├── C6 Bank ✅
│   ├── Banco Original
│   ├── Neon
│   └── Banco PAN
│
├── Corretoras de Investimentos
│   ├── XP Investimentos ✅
│   ├── BTG Pactual ✅
│   ├── Rico
│   ├── Clear
│   ├── Genial Investimentos
│   ├── Ágora Investimentos
│   ├── Toro Investimentos
│   └── Warren Investimentos
│
└── Instituições de Pagamento
    ├── Mercado Pago ✅
    ├── PagBank
    ├── PicPay ✅
    ├── 99Pay
    └── RecargaPay
```

```text
Tipos de Investimento 📈

├── Renda Fixa
│   ├── Conta de Investimento ✅
│   ├── Caderneta de Poupança ✅
│   ├── CDB ✅
│   ├── LCI ✅
│   ├── LCA ✅
│   ├── Tesouro Direto ✅
│   └── Previdência Privada ✅
│
├── Fundos
│   ├── Fundo de Investimento ✅
│   └── Fundo de Investimento Imobiliário (FII) ✅
│
├── Renda Variável
│   ├── Ações ✅
│   └── ETFs
│
└── Ativos Digitais
    └── Criptoativos ✅
```

```text
Carteira Pessoal 🙎‍♂️
├── Reserva de Emergência 
│   ├── Fundo para 6 meses de despesas
│   ├── Manutenção da casa
│   └── Manutenção do veículo
│
├── Reserva de Oportunidade
│   ├── Comprar ações em queda
│   ├── Investir em um novo negócio
│   └── Aproveitar uma promoção de alto valor
│
├── Reserva Patrimonial
│   ├── Comprar um imóvel
│   ├── Comprar um veículo
│   └── Fazer uma viagem internacional
│
└── Reserva de Aposentadoria
    ├── Atingir renda passiva mensal
    ├── Formar patrimônio para aposentadoria
    └── Complementar a previdência

Carteira Familiar 👨‍👩‍👦
├── Reserva Familiar de Emergência
│   ├── Fundo de emergência familiar
│   ├── Despesas médicas inesperadas
│   └── Reforma urgente da residência
│
├── Reserva de Projetos Familiares
│   ├── Viagem em família
│   ├── Compra da casa própria
│   └── Casamento de um filho
│
├── Reserva Patrimonial Familiar
│   ├── Aquisição de imóvel para investimento
│   ├── Compra de uma chácara
│   └── Formação de patrimônio para herança
│
└── Reserva Educacional
    ├── Faculdade dos filhos
    ├── Curso de idiomas
    └── Intercâmbio internacional

Carteira Empresarial 📊
├── Reserva Operacional
│   ├── Capital de giro
│   ├── Pagamento da folha salarial
│   └── Pagamento de fornecedores
│
├── Reserva de Expansão
│   ├── Abrir uma nova unidade
│   ├── Contratar novos colaboradores
│   └── Expandir para uma nova região
│
├── Reserva de Investimento
│   ├── Compra de equipamentos
│   ├── Implantação de novo sistema
│   └── Modernização da infraestrutura
│
└── Reserva Tributária
    ├── Pagamento de impostos
    ├── Pagamento de encargos trabalhistas
    └── Renovação de licenças e taxas
```